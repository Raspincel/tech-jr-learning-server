...
export default async function verifyEmailValidity(req: Request, res: Response, next: NextFunction) {
    ...
    if (!isValidEmail) throw new AppError(401, "You must use your academic email to register :(", {
        message: "User tried to register with an email that does not belong to the IFMA domain (does not end with @acad.ifma.edu.br)",
        email
    })
